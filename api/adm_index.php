...
$arrRecord = array();
$var_consulta = "SELECT * FROM record LIMIT 1";

//$var_resultado = $conexion->query($var_consulta);
$qTMP = mysqli_query($link, $var_consulta);
while ($rTMP = mysqli_fetch_assoc($qTMP)) {
    $arrRecord[$rTMP["id"]]["id"] = $rTMP["id"];
    $arrRecord[$rTMP["id"]]["empresa"] = $rTMP["empresa"];
    $arrRecord[$rTMP["id"]]["cliente"] = $rTMP["cliente"];
...
if (is_array($arrRecord) && (count($arrRecord) > 0)) {
    reset($arrRecord);
    foreach ($arrRecord as $rTMP['key'] => $rTMP['value']) { 

            $empresa = $rTMP["value"]['empresa'];
            $cliente = $rTMP["value"]['cliente'];
